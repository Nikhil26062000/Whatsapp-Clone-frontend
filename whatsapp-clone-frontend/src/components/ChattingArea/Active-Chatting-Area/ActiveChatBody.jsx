...
//! Before:
//! The ActiveChatBody component was a regular functional component which cannot accept refs directly.

//? After:
//? We updated ActiveChatBody to use React.forwardRef so it can accept a ref from its parent component, ActiveChatArea.
...
const ActiveChatBody = forwardRef(({ chatMessage }, ref) => {
  const { account } = useContext(AccountContext);
      ...
      {chatMessage &&
        chatMessage.map((message, index) => (
          <div
            key={index}
            className={`w-fit max-w-[60%] p-2 rounded-lg ${
              account.sub === message.senderId ? "bg-[#005c4b]" : "bg-[#202c33]"
            } mx-10 my-4 ${
              account.sub === message.senderId ? "ml-auto" : "mr-auto"
            }`}
          >
      ...
      <div>
        {message?.msg?.includes(".pdf") ? (
          <div className="w-[500px] h-[100px] flex justify-around items-center">
            ...
            <span className="text-xs pl-5 break-keep flex-shrink-0 block mt-auto bottom-0 right-0">
              <GetAppIcon
                className="bottom-0 right-14 mr-2"
                onClick={(e) => downloadMedia(e, message.msg)}
              />
              {formatDate(message.createdAt)}
            </span>
